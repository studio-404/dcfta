...
class Image extends Controller
{
	...
	public function loadimage()
	{
		try
		{
			...
			$file_path = 'public/_temporaty/' . $cache_file_name;
			ini_set("gd.jpeg_ignore_warning", 1);
			if (file_exists($file_path))
			{
			    header('Content-type:image/png');
				readfile(Config::WEBSITE. $file_path);
			}
			else
			{
				$src = 'public/_temporaty/' . sha1('to_crop_' . $img . $w . $h) . '.' . $ext;
				$without = str_replace(Config::WEBSITE, "", $img);
				@copy($without, $src);
